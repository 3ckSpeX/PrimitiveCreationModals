...
from .primitive_Plane import UpdatePlane
from .primitive_Box import UpdateBox
...
def AddObject(self, context):
    ...
    if self.state == 0 or self.state == 1 :
        bpy.ops.mesh.add_primplane(location=self.loc, rotation=self.rot, scale=self.scale, center=self.center, seg=self.seg)
    #State 2 should be state 1 plus it's Z Axis i.e. state 1 = plane, state 2 = box
    if self.state == 2:
        bpy.ops.mesh.add_primbox(location=self.loc, rotation=self.rot, scale=self.scale, center=self.center, seg=self.seg)
...
def UndoRedoObject(self, context):
    ...
    if self.mode == 'EDIT':
        ...
        if self.state == 0 or self.state == 1:
            UpdatePlane(cont=context, scale=self.scale, center=self.center, seg=self.seg)
        if self.state == 2:
            UpdateBox(cont=context, scale=self.scale, center=self.center, seg=self.seg)
            ...
            if self.state == 1:
                ...
                if event.type == 'WHEELDOWNMOUSE':    
                    if not event.ctrl:
                        self.seg[0] -= 1
                        self.seg[1] -= 1
                        self.seg[0] = max(self.seg[0], 0)
                        self.seg[1] = max(self.seg[1], 0)
                    else:
                        if self.snapInc <= 1:
                            self.snapInc -= 0.25
                        else:
                ...
                if self.seg[0] > 0 or self.seg[1] > 0:
                    if not self.mode == 'EDIT':
                        self.wire = True
...
def register():
    bpy.utils.register_class(BoxCreationModalOperator)

def unregister():
    bpy.utils.unregister_class(BoxCreationModalOperator)
